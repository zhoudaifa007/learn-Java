...
public class Leetcode0105 {
    ...
    private TreeNode helper(int[] preorder, int s1, int e1, int[] inorder, int s2, int e2) {
        if(s1 > e1 || s2 > e2 || s1 > preorder.length - 1 || s2 > preorder.length - 1) {
            return null;
        }
        ...
        treeNode.left = helper(preorder, s1 + 1, s1 + offset, inorder, s2, index - 1);
        treeNode.right = helper(preorder, s1 + offset + 1, e1, inorder, index + 1, e2);
        return treeNode;
    }
}
